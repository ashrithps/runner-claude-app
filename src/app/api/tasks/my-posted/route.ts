...
export async function GET(request: NextRequest) {
  try {
    ...
    const tasks = DatabaseOperations.getTasksByPosterId(sessionInfo.userId)
    ...
    const tasksWithNames = tasks.map((task) => {
      ...
      const runner = task.runner_id ? DatabaseOperations.getUserById(task.runner_id) : null
    ...
    return NextResponse.json(
      { error: 'Failed to load posted tasks' },
      { status: 500 }
    )
  }
}